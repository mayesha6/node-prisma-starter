...
const calculateEndDate = (startDate, interval, intervalCount) => {
    ...
    switch (interval) {
        ...
        case "year":
            ...
            break;
        default:
            throw new AppError(httpStatus.BAD_REQUEST, `Unsupported interval: ${interval}`);
    }
    return endDate;
};
const handlePaymentIntentSucceeded = async (paymentIntent) => {
    ...
    if (!payment) {
        throw new AppError(httpStatus.NOT_FOUND, `Payment not found for ID: ${paymentIntent.id}`);
    }
    if (!payment.plan) {
        throw new AppError(httpStatus.NOT_FOUND, "Plan not found for this subscription");
    }
    if (paymentIntent.status !== "succeeded") {
        throw new AppError(httpStatus.BAD_REQUEST, "Payment intent is not in succeeded state");
    }
